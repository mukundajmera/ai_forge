...
# --- MONKEYPATCH FOR OLDER GGUF PACKAGE ---
try:
    _missing_archs = [
        "AFMOE", "APERTUS", "ARCEE", "ARCTIC", "ARWKV7", "BAICHUAN", "BAILINGMOE", "BAILINGMOE2", "BERT", "BITNET", 
        "BLOOM", "CHAMELEON", "CHATGLM", "CODESHELL", "COGVLM", "COHERE2", "COMMAND_R", "DBRX", "DECI", "DEEPSEEK", 
        "DEEPSEEK2", "DOTS1", "DREAM", "ERNIE4_5", "ERNIE4_5_MOE", "EXAONE", "EXAONE4", "EXAONE_MOE", "FALCON", 
        "FALCON_H1", "GEMMA", "GEMMA2", "GEMMA3", "GEMMA3N", "GEMMA_EMBEDDING", "GLM4", "GLM4_MOE", "GPT2", 
        "GPTNEOX", "GPT_OSS", "GRANITE", "GRANITE_HYBRID", "GRANITE_MOE", "GROK", "GROVEMOE", "HUNYUAN_DENSE", 
        "HUNYUAN_MOE", "INTERNLM2", "JAIS", "JAMBA", "JINA_BERT_V2", "JINA_BERT_V3", "LFM2", "LFM2MOE", "LLADA", 
        "LLADA_MOE", "LLAMA", "LLAMA4", "LLAMA_EMBED", "MAINCODER", "MAMBA", "MAMBA2", "MIMO2", "MINICPM", 
        "MINICPM3", "MINIMAXM2", "MISTRAL3", "MMPROJ", "MODERN_BERT", "MPT", "NEMOTRON", "NEMOTRON_H", 
        "NEMOTRON_H_MOE", "NEO_BERT", "NOMIC_BERT", "NOMIC_BERT_MOE", "OLMO", "OLMO2", "OLMOE", "OPENELM", 
        "ORION", "PANGU_EMBED", "PHI2", "PHI3", "PHIMOE", "PLAMO", "PLAMO2", "PLAMO3", "PLM", "QWEN", "QWEN2", 
        "QWEN2MOE", "QWEN2VL", "QWEN3", "QWEN3MOE", "QWEN3NEXT", "QWEN3VL", "QWEN3VLMOE", "REFACT", "RND1", 
        "RWKV6", "RWKV6QWEN2", "RWKV7", "SEED_OSS", "SMALLTHINKER", "SMOLLM3", "STABLELM", "STARCODER", 
        "STARCODER2", "T5", "T5ENCODER", "WAVTOKENIZER_DEC", "XVERSE"
    ]
    for _i, _arch in enumerate(_missing_archs):
        if not hasattr(gguf.MODEL_ARCH, _arch):
            setattr(gguf.MODEL_ARCH, _arch, 9000 + _i)

    _missing_tensors = [
        "ALTUP_CORRECT_COEF", "ALTUP_PREDICT_COEF", "ALTUP_ROUTER", "ATTN_GATE", "ATTN_K", "ATTN_OUT", "ATTN_Q", 
        "ATTN_QKV", "ATTN_V", "A_ENC_EMBD_POS", "CLS_OUT", "FFN_DOWN", "FFN_GATE", "FFN_GATE_EXP", "FFN_GATE_INP", 
        "FFN_GATE_SHEXP", "FFN_UP", "FFN_UP_EXP", "FFN_UP_SHEXP", "LAUREL_L", "LAUREL_R", "OUTPUT", 
        "PER_LAYER_TOKEN_EMBD", "POSNET_NORM1", "POSNET_NORM2", "POS_EMBD", "ROPE_FACTORS_LONG", 
        "ROPE_FACTORS_SHORT", "ROPE_FREQS", "SHORTCONV_CONV", "SSM_A", "SSM_CONV1D", "SSM_D", "SSM_NORM", 
        "TIME_MIX_DECAY_W1", "TIME_MIX_DECAY_W2", "TIME_MIX_FIRST", "TIME_MIX_LERP_FUSED", "TIME_MIX_W1", 
        "TIME_MIX_W2", "TOKEN_EMBD", "TOKEN_TYPES", "V_DS_FC1", "V_DS_FC2", "V_DS_NORM", "V_ENC_EMBD_PATCH", 
        "V_ENC_EMBD_POS", "V_MMPROJ", "V_MMPROJ_FC", "V_POST_NORM", "V_TOK_EMBD_IMG_BREAK"
    ]
    for _i, _tensor in enumerate(_missing_tensors):
        if not hasattr(gguf.MODEL_TENSOR, _tensor):
            setattr(gguf.MODEL_TENSOR, _tensor, 8000 + _i)

    # Shim for SafetensorsLocal
    if not hasattr(gguf.utility, 'SafetensorsLocal'):
        from safetensors.torch import load_file
        import contextlib
        print("Monkeypatching SafetensorsLocal...")
        class _SafetensorsLocalShim:
            ...
            def __exit__(self, *args):
                pass
        gguf.utility.SafetensorsLocal = _SafetensorsLocalShim

except Exception as _e:
    print(f"Warning: Monkeypatch failed: {_e}")
# ------------------------------------------
        ...
        for part_name in part_names:
            ...
            with ctx as model_part:
                ...
                for name in model_part.keys():
                    if is_safetensors and not isinstance(model_part[name], torch.Tensor):
