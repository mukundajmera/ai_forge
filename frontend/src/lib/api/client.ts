...
class APIClient {
    ...
    async startFineTune(config: FineTuneConfig | Record<string, any>): Promise<FineTuneResponse> {
        // Accept either camelCase (from types) or snake_case (from UI dialog) config
        // Normalize to snake_case for backend if camelCase is detected
        const backendConfig = 'projectName' in config ? {
